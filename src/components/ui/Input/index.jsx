...
const Input = React.forwardRef(({ name, type, label, leftIcon: LIcon, rightIcon: RIcon, error, ...props }, ref) => {
   const { handleToggle, toggle: show } = type === "password" && useToggle(false);

   const inputType = useMemo(() => (type === "password" ? (show ? "text" : "password") : type), [type, show]);

   return (
      <>
         <div className={`${name}_input flex flex-col gap-[5px] w-[100%]`}>
            {label && <label className="font-PoppinsBold text-[#2b3445] text-[15px] sm:text-[16px]">{label}</label>}
            <div
               className={cn(
               ...
               {type === "password" && (
                  <button
                     ...
                     {show ? <LuEye className="text-[19px] sm:text-[20px]" /> : <LuEyeOff className="text-[19px] sm:text-[20px]" />}
                  </button>
               )}
            </div>
            {error && <span className="error text-[red] font-Sans text-[14px] md:text-[15px]">{error}</span>}
         </div>
      </>
   );
});
