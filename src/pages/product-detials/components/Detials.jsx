...
const Details = ({ product }) => {
   const { name, description, numOfReviews, price, ratings, stock, images, reviews, category, _id } = product;

   const { handleAddToCart, isAddCart } = useAddToCart();
   const { count, decrement, increment } = useCounter(localStorageItem(_id)?.quantity || 0, 1, stock);
         ...
         <div className="container py-[50px] md:py-[80px] grid grid-cols-1 md:grid-cols-2 items-center justify-center gap-[20px] relative">
            ...
            <div className="flex items-center justify-start  md:justify-center w-full">
               <Image
                  src={images[0]?.url}
                  alt={name}
                  className={" w-full max-w-[300px] md:max-w-[500px] h-[300px] sm:h-[330px!important] mt-[20px] sm:mt-[0px] object-contain  "}
               />
            </div>
            <div className=" flex flex-col gap-[10px] md:gap-[15px] mt-[20px] ">
               <div className=" ">
                  <h2 className="text-[30px] font-bold font-PoppinsBold text-[#2B3445]">{capitalize(String(name))}</h2>
               </div>
               {/* Ratings */}
               ...
               <div className="flex items-center justify-start gap-[5px] font-Sans text-[16px]">
                  Category: <span className="font-SansBold">{capitalize(String(category))}</span>
               </div>
               <div className="flex items-center justify-start">
                  <span className="text-[#D23F57] font-SansBold text-[25px]">${price} </span>
               </div>
               ...
               <div className="flex items-center justify-start gap-[6px] text-[17px] font-PoppinsBold ">
                  Stock:
                  <span className={cn(stock < 1 ? "text-[#D23F57]" : "text-[green]")}>{stock < 1 ? "OutOfStock" : "InStock"}</span>
               </div>
               ...
               <div className="flex items-center justify-start gap-[6px] text-[17px] font-PoppinsBold ">
                  ...
                  <p className="text-gray-500 font-Sans text-[15px] line-clamp-3">{description}</p>
               </div>
