...
const Header = () => {
   ...
   return (
      <>
         <header
            id="header"
            className={cn(`w-[100%] border-b border-solid border-[#d1d5db] h-[70px] md:h-[74px]`, headerScroll ? "header_animation" : "")}>
               ...
               <ul className=" gap-[34px] font-PoppinsBold hidden md:flex">
                  {NAV.Links.map((item, index) => (
                     <Item
                        ...
                        path={item.path}
                        key={index}
                        className={pathname === item.path ? "opacity-1" : "opacity-[0.8]"}
                     />
                  ))}
               </ul>
                  ...
                  {NAV.Icons.map((item, index) =>
                     item.path === "/cart" ? (
                        ...
                        <Item
                           {...item}
                           key={index}
                           className={cn("header_icon", userInfo && item.path === "/login" ? "hidden" : "flex")}
                        />
                     )
                  )}
                  {userInfo && (
